...
if (isset($_POST['Enviar'])) {
    ...
    if (strlen($nombre) >= 1 && strlen($numeroContacto) >= 1) {
        ...
        // Verificación si el proveedor ya existe
        $stmtVerificacion = mysqli_prepare($CONEXION, "SELECT * FROM proveedor WHERE Nombre_Proveedor = ?");
        ...
        // Comprobar si ya existe un proveedor con el mismo nombre
        if (mysqli_stmt_num_rows($stmtVerificacion) > 0) {
            agregarToast('error', 'Error', '¡Este proveedor ya existe!');
        } else {
